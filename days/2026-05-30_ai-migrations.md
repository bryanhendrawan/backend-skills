...
<!-- Slide 1 -->
<!-- cover-terminal: ~/backend-service $ migrate —describe —generate —verify -->

**Theme**: Schema change yang aman: AI yang generate, tim yang verify rollback.

## The Problem
<!-- Slide 2 -->

Schema change masuk sprint backlog, tapi engineer ragu. Migration manual sering lupa down migration atau index pendukung.

## How It Works

1. Deskripsikan perubahan ke AI: "tambah kolom email unique ke tabel users, buat index-nya."
<!-- Slide 3 -->

<!-- chat-bubble -->
[PROMPT] Tambah kolom email NOT NULL UNIQUE ke tabel users, lengkap dengan rollback.
[RESPONSE] ALTER TABLE users ADD COLUMN email TEXT NOT NULL; CREATE UNIQUE INDEX idx_users_email ON users(email);
<!-- /chat-bubble -->

2. AI generate up dan down migration sekaligus. Review file sebelum apply ke DB mana pun.
<!-- Slide 4 -->

<!-- comparison-window: Review checklist -->
[BAD] langsung apply ke dev tanpa baca migration file.
↳ down migration hilang, rollback gagal saat incident.
[GOOD] review naming, index, dan down migration dulu.
↳ rollback aman, tidak ada schema drift di production.
<!-- /comparison-window -->
...
**Migration yang aman bukan soal siapa yang nulis. Tapi siapa yang verify rollback.**

## Try It Today
<!-- Slide 7 -->

Buka sprint backlog tim kamu. Ambil satu schema change yang pending. Prompt AI, generate migration, review sebelum commit.
...
- Cover terminal prompt (left-aligned above title, monospace): ~/backend-service $ migrate —describe —generate —verify
...
- Subheading: Schema change yang aman: AI yang generate, tim yang verify rollback. — 18–22px soft lavender, below heading
...
- Headline: Schema change masuk sprint backlog, tapi engineer ragu. Migration manual sering lupa down migration atau index pendukung. — H2, 44–52px bold white, left-aligned
...
- Body: Deskripsikan perubahan ke AI: "tambah kolom email unique ke tabel users, buat index-nya." — 24–28px white, left-aligned to the right of the badge
- Chat panel (left-aligned):
  - Panel: #141128, rounded 8px, border #1E1B3A
  - [PROMPT] row: "You" label in amber (#E8955A) 12px monospace; text "Tambah kolom email NOT NULL UNIQUE ke tabel users, lengkap dengan rollback." in white 16px; 2px amber left border accent
  - [RESPONSE] row: "AI" label in cyan (#00D4FF) 12px monospace; text "ALTER TABLE users ADD COLUMN email TEXT NOT NULL; CREATE UNIQUE INDEX idx_users_email ON users(email);" in white 16px; 2px cyan left border accent
  - 8px gap between rows; tight vertical padding
...
- Body: AI generate up dan down migration sekaligus. Review file sebelum apply ke DB mana pun. — 24–28px white, left-aligned to the right of the badge
- Comparison window (left-aligned):
  - Panel: #141128, rounded 8px, border #1E1B3A, chrome dots top-left
  - Header: Review checklist in soft lavender monospace
...
- Statement: Migration yang aman bukan soal siapa yang nulis. Tapi siapa yang verify rollback. — H2, 44–52px bold white, left-aligned
...
- Action: Buka sprint backlog tim kamu. Ambil satu schema change yang pending. Prompt AI, generate migration, review sebelum commit. — H2, 44–52px bold white, left-aligned
